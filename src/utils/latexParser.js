...
function parseProjects(content) {
  ...
  while ((match = itemRegex.exec(content)) !== null) {
    ...
    const linkMatch = block.match(/\\href\{([^}]*)\}\{(?:Repo )?LINK\}/i);
    const liveLinkMatch = block.match(/\\href\{([^}]*)\}\{(?:Deployed|Live) LINK\}/i);
    ...
    description = description.replace(/\|\s*(?:Repo )?LINK/gi, '').replace(/\|\s*(?:Deployed|Live) LINK/gi, '').replace(/^[\s:|-]+/, '').trim();
